...
- **E**: **技能树**。打开技能加点界面，查看和升级技能。
- **P**: **市场价格/交易**。打开市场价格表（仅查看），在商人处可进行交易。
- **Tab**: **跳过时间**。作弊功能，直接跳到第二天早晨（调试用）。
...
| 按键             | 物品名称           | 功能描述                        | 操作方法                                                  |
| :--------------- | :----------------- | :------------------------------ | :-------------------------------------------------------- |
| **1**      | **锄头**     | 开垦土地，将草地变为耕地。      | 面向目标地块，按**J** 键。                          |
| **2**      | **喷壶**     | 给耕地浇水，作物生长必须。      | 面向耕地，按**J** 或 **K** 键。               |
| **3**      | **镰刀**     | 收获已成熟的作物。              | 面向成熟作物，按**J** 键。                          |
| **4**      | **斧头**     | 砍伐树木，清理障碍。            | 面向树木（需靠近），按**J** 键。砍树需要约2秒时间。 |
| **5**      | **钓鱼竿**   | 在水边钓鱼。                    | 详见下方“海滩与钓鱼”章节。                              |
| **6-0, -** | **各类种子** | 种植作物 (萝卜、土豆、玉米等)。 | 面向已开垦且未种植的耕地，按**J** 键。              |
| **(掉落)** | **剑**       | 攻击怪物。                      | 面向怪物按**J** 键。                                |
...
- **查看天气**:
  ...
  - **未来预报**: 回到小屋，走到 **电视机** 前（进门左手边），按 **J** 键进行交互，可查看未来 7 天的天气预报。提前规划耕作更加高效！
...
1. **进入矿洞**: 在农场地图找到电梯入口，**靠近后**按 **M** 键打开楼层选择界面。
2. **战斗**:
   - 矿洞中会生成 **史莱姆** 和 **僵尸**。
   - **史莱姆**: 移动较快，血量低。
   - **僵尸**: 移动较慢，血量高，攻击强。
   - 按 **J** 键使用武器进行攻击。
   - 随着层数深入 (楼层 1 -> 5)，怪物会变得更强。
3. **挖矿**:
...
1. **农业**: 耕种、收获提升。升级可增加收获额外产出概率。
2. **采矿**: 挖矿提升。升级可减少挖矿时的体力消耗。
3. **钓鱼**: 钓鱼提升。升级可增加鱼的价格和钓鱼容错率。
4. **烹饪**: (待实装) 制作食物提升。
