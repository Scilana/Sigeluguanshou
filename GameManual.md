...
- **M**: **电梯交互键**。当靠近农场右上角的电梯入口时，按下可打开楼层选择界面。
...
- **K 键**: **放置储物箱**。在农场中，将鼠标指向空地，按下 **K** 键可在该位置放置一个储物箱（消耗50木材 + 100金币）。
- **Space (空格键)**: **开启箱子**。当靠近宝箱或储物箱时按下。
- **鼠标左键**: 
    - **背包管理**: 打开背包后，**单击**一个格子选中，再**单击**另一个格子可交换物品位置。
- **转移物品**: 在打开箱子时，按 **J** 键将选中物品转移到另一侧。如果是多个物品，会弹出数量输入框（支持键盘数字输入）。
    - **钓鱼**: 仅当手持 **钓鱼竿** 时有效。
...
1.  **放置**:
    -   确保背包中有 **50 木材** 和 **100 金币**。
    -   将鼠标悬停在农场的空地上（不能有作物或障碍物）。
    -   按 **K** 键放置一个新的储物箱。
    -   *注意：放置后无法移动，请谨慎选址。*
2.  **使用**:
    -   走到储物箱旁边（人物需靠近）。
...
3.  **物品管理**:
    -   箱子拥有 **32格** 额外的储存空间。
    -   打开背包后按 **J** 键，可将选中物品在“背包”与“箱子”之间转移。
    -   **碰撞体积**: 箱子放置后具有实体，**人物无法穿过**（如刚放置时重叠，允许走出）。
