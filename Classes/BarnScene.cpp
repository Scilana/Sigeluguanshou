...
Vec2 getDoorTileFromWarp(MapLayer* mapLayer) {
  if (!mapLayer) return kFallbackDoorTile;

  auto tmxMap = mapLayer->getTMXMap();
  if (!tmxMap) return kFallbackDoorTile;

  auto props = tmxMap->getProperties();
  auto it = props.find("Warp");
  if (it == props.end()) return kFallbackDoorTile;

  std::istringstream iss(it->second.asString());
  int x = 0;
  int y = 0;
  if (!(iss >> x >> y)) return kFallbackDoorTile;
...
void BarnScene::initPlayer() {
  ...
  if (mapLayer_) {
    ...
    exitRadius_ =
        std::max(40.0f, std::max(tileSize.width, tileSize.height) * 2.0f);
...
void BarnScene::initUI() {
  ...
  auto helpLabel =
      Label::createWithSystemFont("B: Bag | ENTER: Exit", "Arial", 14);
...
void BarnScene::initToolbarUI() {
  ...
  toolbarUI_->setPosition(
      Vec2(origin.x + visibleSize.width * 0.5f, origin.y + 6.0f));
...
void BarnScene::updateCamera() {
  ...
  if (mapLayer_) {
    ...
    if (mapSize.width < visibleSize.width)
      minX = maxX = mapOrigin.x + mapSize.width / 2;
    if (mapSize.height < visibleSize.height)
      minY = maxY = mapOrigin.y + mapSize.height / 2;
...
void BarnScene::updateUI() {
  auto tm = TimeManager::getInstance();
  if (tm && timeLabel_) {
    timeLabel_->setString(StringUtils::format("Day %d, %02d:%02d", tm->getDay(),
...
void BarnScene::onInventoryClosed() { inventoryUI_ = nullptr; }

void BarnScene::showActionMessage(const std::string& text,
                                  const Color3B& color) {
