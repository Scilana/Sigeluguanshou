...
void Monster::initStats() {
  ...
  // 根据楼层等级增强属性 (降低增长幅度)
  float multiplier =
      1.0f + (floorLevel_ - 1) * 0.2f;  // 每层增加 20%，之前是 50%

  attackPower_ =
      static_cast<int>(attackPower_ * 0.6f * multiplier);  // 基础伤害打6折
...
void Monster::initDisplay() {
  ...
  if (spritePath.empty()) {
    ...
    // 绘制白色方块
    Vec2 vertices[] = {Vec2(-12, -12), Vec2(12, -12), Vec2(12, 12),
                       Vec2(-12, 12)};
...
void Monster::updateAI(float delta) {
  if (!targetPlayer_ || isDead()) return;
...
void Monster::moveTowards(const Vec2& targetPos, float delta) {
  ...
  if (mapLayer_) {
    ...
    if (newPos.x < 0 || newPos.x > mapSize.width || newPos.y < 0 ||
        newPos.y > mapSize.height) {
      return;  // 超出地图边界，不移动
    }
...
void Monster::takeDamage(int damage) {
  ...
  // 受伤闪烁效果
  auto blink = Sequence::create(TintTo::create(0.1f, 255, 0, 0),
                                TintTo::create(0.1f, 255, 255, 255), nullptr);
...
void Monster::attackPlayer(Player* player) {
  if (!player || currentAttackCooldown_ > 0) return;
  ...
  // 攻击动画（简单的缩放效果）
  auto attackAnim = Sequence::create(ScaleTo::create(0.1f, 1.2f),
