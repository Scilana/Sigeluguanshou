...
bool FarmManager::init(MapLayer* mapLayer) {
  ...
  initCropDefs();
  tiles_.resize(
      static_cast<size_t>(mapSizeTiles_.width * mapSizeTiles_.height));
  ...
  if (SaveManager::getInstance()->hasSaveFile() &&
      SaveManager::getInstance()->loadGame(saveData)) {
...
int FarmManager::getHour() const {
  return TimeManager::getInstance()->getHour();
}
int FarmManager::getMinute() const {
  return TimeManager::getInstance()->getMinute();
}
...
FarmManager::ActionResult FarmManager::tillTile(const Vec2& tileCoord) {
  ...
  auto& tile = tiles_[static_cast<size_t>(tileCoord.y * mapSizeTiles_.width +
                                          tileCoord.x)];
...
bool FarmManager::isValidTile(const Vec2& tileCoord) const {
  return tileCoord.x >= 0 && tileCoord.y >= 0 &&
         tileCoord.x < mapSizeTiles_.width &&
         tileCoord.y < mapSizeTiles_.height;
}
...
void FarmManager::redrawOverlay() {
  ...
  for (int y = 0; y < mapSizeTiles_.height; ++y) {
    for (int x = 0; x < mapSizeTiles_.width; ++x) {
      const auto& tile =
          tiles_[static_cast<size_t>(y * mapSizeTiles_.width + x)];
      ...
      if (soilSprite) {
        ...
        soilSprite->setScale(
            soilSprite->getContentSize().height <= 16.0f ? 2.0f : 1.0f);
      ...
      if (tile.hasCrop) {
        auto sprite =
            Sprite::create(getCropTextureName(tile.cropId, tile.stage));
        if (sprite) {
          sprite->getTexture()->setAliasTexParameters();
          sprite->setScale(sprite->getContentSize().height <= 16.0f ? 2.0f
                                                                    : 1.0f);
