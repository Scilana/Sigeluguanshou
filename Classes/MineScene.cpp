...
bool syncSaveInventoryAndSkills(InventoryManager* inventory) {
  ...
  if (!saveMgr || !saveMgr->hasSaveFile()) return false;

  SaveManager::SaveData data;
  if (!saveMgr->loadGame(data)) return false;
...
MineScene* MineScene::createScene(InventoryManager* inventory,
                                  int currentFloor) {
      ...
      if (mineLayer_->isWalkable(mapCenter)) {
        ...
        for (int radius = 16; radius < 320 && !foundWalkable; radius += 16) {
          ...
          for (int angle = 0; angle < 360 && !foundWalkable; angle += 45) {
            ...
            Vec2 testPos =
                mapCenter + Vec2(cos(rad) * radius, sin(rad) * radius);

            //
            if (testPos.x >= 0 && testPos.x < mapSize.width && testPos.y >= 0 &&
                testPos.y < mapSize.height) {
...
void MineScene::initUI() {
  ...
  //
  auto topBar =
      LayerColor::create(Color4B(0, 0, 0, 180), visibleSize.width, 40);
  ...
  if (tm) {
    timeStr = StringUtils::format("Day %d, %02d:%02d", tm->getDay(),
  ...
  itemLabel_->setPosition(Vec2(origin.x + visibleSize.width - 20,
                               origin.y + visibleSize.height - 20));
  ...
  //
  auto tipLabel =
      Label::createWithSystemFont("(Keys 1-8 to switch)", "Arial", 12);
  tipLabel->setAnchorPoint(Vec2(1, 0.5));
  tipLabel->setPosition(Vec2(origin.x + visibleSize.width - 20,
...
void MineScene::initElevator() {
  ...
  if (!elevatorSprite_) {
    ...
    //
    draw->drawSolidRect(Vec2(-innerSize, -innerSize),
    ...
    Vec2 glowPoints[] = {
        Vec2(-innerSize, -innerSize), Vec2(innerSize, -innerSize),
        Vec2(innerSize, innerSize), Vec2(-innerSize, innerSize)};
...
void MineScene::initChests() {
  ...
  if (chestCount > 0) {
    ...
    if (TimeManager::getInstance())
      dayCount = TimeManager::getInstance()->getDay();
...
void MineScene::update(float delta) {
  ...
  if (tm) {
    ...
    if (uiLayer_) {
      ...
      if (label) {
        label->setString(StringUtils::format("Day %d, %02d:%02d", tm->getDay(),
                                             tm->getHour(), tm->getMinute()));
      }
    }
...
void MineScene::updateCamera() {
  ...
  if (uiLayer_) {
    Vec2 uiPos =
        Vec2(newPos.x - Director::getInstance()->getVisibleSize().width / 2,
...
void MineScene::updateUI() {
  ...
  if (inventory_ && itemLabel_) {
    ...
    itemLabel_->setString(StringUtils::format(
        "[%d] %s", selectedItemIndex_ == 9 ? 0 : selectedItemIndex_ + 1,
        name.c_str()));
  }
  ...
  if (!healthLabel_ && uiLayer_) {
    ...
    healthLabel_->setPosition(
        Vec2(origin.x + 20, origin.y + visibleSize.height - 50));
    healthLabel_->setColor(Color3B::RED);
    uiLayer_->addChild(healthLabel_, 1);
  }

  if (healthLabel_ && player_) {
    healthLabel_->setString(
        StringUtils::format("HP: %d/%d", player_->getHp(), 100));
  }
  refreshToolbarUI();
}
  ...
  if (inventory_) {
    ...
    for (int i = 0; i < inventory_->getSlotCount(); ++i) {
      ...
      if (t == ItemType::ITEM_WoodenSword || t == ItemType::ITEM_IronSword ||
          t == ItemType::ITEM_GoldSword || t == ItemType::ITEM_DiamondSword)
        hasSword = true;
    ...
    if (mineLayer_->isMineralAt(targetTile)) {
      ...
      if (delay > 0) {
        auto seq = Sequence::create(
            DelayTime::create(delay), CallFunc::create([this, targetTile]() {
...
void MineScene::handleAttackAction() {
  ...
  if (inventory_) {
    ...
    if (item == ItemType::ITEM_WoodenSword ||
        item == ItemType::ITEM_IronSword || item == ItemType::ITEM_GoldSword ||
        item == ItemType::ITEM_DiamondSword) {
      attackDamage = Weapon::getWeaponAttackPower(item);
      attackRange = Weapon::getWeaponAttackRange(item);
    } else if (item == ItemType::Pickaxe || item == ItemType::Axe ||
...
void MineScene::showActionMessage(const std::string& text,
                                  const Color3B& color) {
  ...
  auto seq =
      Sequence::create(DelayTime::create(0.5f), FadeOut::create(1.0f), nullptr);
...
void MineScene::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
  switch (keyCode) {
    ...
    case EventKeyboard::KeyCode::KEY_M:
      if (elevatorSprite_) {
        float dist =
            player_->getPosition().distance(elevatorSprite_->getPosition());
        if (dist < 60.0f)  //
        {
          showElevatorUI();
        } else {
...
void MineScene::goToPreviousFloor() {
  ...
  Director::getInstance()->replaceScene(
      TransitionFade::create(0.5f, prevScene));
}
...
void MineScene::goToNextFloor() {
  ...
  Director::getInstance()->replaceScene(
      TransitionFade::create(0.5f, nextScene));
}
...
void MineScene::initWishingWell() {
  ...
  //
  auto label =
      Label::createWithSystemFont("Wishing Well\n(Press K)", "Arial", 12);
...
void MineScene::handleWishAction() {
  ...
  if (inventory_->removeItem(currentItem, 1)) {
    ...
    wishingWell_->runAction(Sequence::create(
        ScaleTo::create(0.1f, 1.2f), ScaleTo::create(0.1f, 1.0f), nullptr));
    ...
    } else if (randVal < 70)  // 40%  /
    {
      if (rand() % 2 == 0) {
        ...
        showActionMessage(StringUtils::format("Well grants %d Gold!", gold),
                          Color3B::YELLOW);
      } else {
    ...
    {
      //
      ItemType rewards[] = {ItemType::GoldOre, ItemType::ITEM_DiamondSword,
                            ItemType::ITEM_GoldSword};
      ItemType reward = rewards[rand() % 3];

      //
      if ((reward == ItemType::ITEM_DiamondSword ||
           reward == ItemType::ITEM_GoldSword) &&
          inventory_->hasItem(reward, 1)) {
        int gold = 500;
        inventory_->addMoney(gold);
        showActionMessage("Great fortune! (500 Gold)", Color3B::ORANGE);
      } else {
        if (inventory_->addItem(reward, 1)) {
          showActionMessage(StringUtils::format(
                                "Received %s!",
                                InventoryManager::getItemName(reward).c_str()),
                            Color3B::MAGENTA);
        } else {
          //
...
void MineScene::showElevatorUI() {
  ...
  if (elevatorUI_) {
    elevatorUI_->setFloorSelectCallback(
        CC_CALLBACK_1(MineScene::onElevatorFloorSelected, this));
    elevatorUI_->setCloseCallback(
        CC_CALLBACK_0(MineScene::onElevatorClosed, this));
...
void MineScene::onElevatorClosed() { elevatorUI_ = nullptr; }
