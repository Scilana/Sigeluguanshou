...
bool EnergyBar::init(Player* player) {
  ...
  bg->drawRect(Vec2(-barWidth_ / 2 - 2, -2),
               Vec2(barWidth_ / 2 + 2, barHeight_ + 2), Color4F(0, 0, 0, 0.5f));
  this->addChild(bg, 0);
...
void EnergyBar::update(float delta) { updateBar(); }
...
void EnergyBar::updateBar() {
  ...
  if (currentHeight > 0) {
    barNode_->drawSolidRect(Vec2(-barWidth_ / 2, 0),
                            Vec2(barWidth_ / 2, currentHeight), color);
  }
