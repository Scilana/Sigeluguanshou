...
bool BlacksmithUI::init() {
  ...
  panel_->setPosition(Vec2(origin.x + visibleSize.width / 2,
                           origin.y + visibleSize.height / 2));
  this->addChild(panel_, 1);

  //
  auto border = DrawNode::create();
  border->drawRect(Vec2(0, 0), Vec2(panelW, panelH),
  ...
  auto closeItem =
      MenuItemLabel::create(closeLabel, [this](Ref* sender) { close(); });
  ...
  auto btnRepair =
      MenuItemLabel::create(Label::createWithSystemFont("Repair", "Arial", 20),
                            [this](Ref*) { switchMode(Mode::Repair); });
  btnRepair->setPosition(Vec2(80, panelH - 60));
  auto btnShop =
      MenuItemLabel::create(Label::createWithSystemFont("Shop", "Arial", 20),
                            [this](Ref*) { switchMode(Mode::Shop); });
...
void BlacksmithUI::refreshList() {
  ...
  if (goldLabel_) {
    goldLabel_->setString(
        StringUtils::format("Gold: %d", inventory->getMoney()));
  }
  ...
  if (currentMode_ == Mode::Shop) {
    ...
    for (const auto& item : shopItems) {
      ...
      auto priceLabel = Label::createWithSystemFont(
          StringUtils::format("%d G", item.price), "Arial", 14);
  ...
  for (int i = 0; i < inventory->getSlotCount(); ++i) {
    ...
    if (slot.isTool() && slot.maxDurability > 0 &&
      ...
      repairItems.push_back(
          {i, slot.type, slot.durability, slot.maxDurability, cost});
      totalCost += cost;
    }
  }
  ...
  if (!repairItems.empty()) {
    ...
    auto summaryLabel =
        Label::createWithSystemFont("Repair All Damaged Tools", "Arial", 16);
    ...
    auto repairAllBtn = MenuItemLabel::create(
        repairAllLabel, [this](Ref*) { this->onRepairAllClicked(); });
  ...
  for (const auto& entry : repairItems) {
    ...
    auto durLabel = Label::createWithSystemFont(
        StringUtils::format("Durability: %d/%d", entry.durability,
                            entry.maxDurability),
        "Arial", 14);
    ...
    int cost = entry.cost;
    auto repairBtn =
        MenuItemLabel::create(btnLabel, [this, slotIndex, cost](Ref* sender) {
  ...
  if (repairItems.empty()) {
    auto label = Label::createWithSystemFont("All tools are in good condition!",
                                             "Arial", 18);
...
void BlacksmithUI::onRepairAllClicked() {
  ...
  if (!inventory->removeMoney(totalCost)) return;
