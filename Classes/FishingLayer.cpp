...
void FishingLayer::initUI() {
  ...
  const Vec2 center = Vec2(visibleSize.width / 2, visibleSize.height / 2);
  ...
  if (!fish) {
    ...
    float scale = std::min(targetSize / fish->getContentSize().width,
                           targetSize / fish->getContentSize().height);
    fish->setScale(scale);
...
void FishingLayer::initInput() {
  ...
  keyListener->onKeyPressed = [this](EventKeyboard::KeyCode code,
                                     Event* event) {
    if (code == EventKeyboard::KeyCode::KEY_SPACE ||
        code == EventKeyboard::KeyCode::KEY_C) {
      this->isHolding_ = true;
    }
  };
  keyListener->onKeyReleased = [this](EventKeyboard::KeyCode code,
                                      Event* event) {
    if (code == EventKeyboard::KeyCode::KEY_SPACE ||
        code == EventKeyboard::KeyCode::KEY_C) {
      this->isHolding_ = false;
...
void FishingLayer::updateFishMovement(float delta) {
  ...
  if (moveTimer_ <= 0) {
    // 难度越高，移动范围越广
    float range =
        0.5f + (currentFish_ ? currentFish_->getDifficulty() * 0.4f : 0.2f);
...
void FishingLayer::updateProgress(float delta) {
  ...
  if (bg) {
    ...
    if (fg) {
      ...
      if (catchProgress_ > 0.7f)
        fg->setColor(Color3B::GREEN);
      else if (catchProgress_ > 0.3f)
        fg->setColor(Color3B::YELLOW);
      else
        fg->setColor(Color3B::RED);
    }
  }
...
void FishingLayer::finishFishing(bool success) {
  ...
  auto label =
      Label::createWithSystemFont(success ? "PERFECT!" : "MISS", "Arial", 48);
