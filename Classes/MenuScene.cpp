...
cocos2d::MenuItemImage* MenuScene::createImageButton(
  ...
  if (!checkImageExists(normalImage)) return nullptr;

  std::string selectedPath = selectedImage;
  if (!checkImageExists(selectedImage)) selectedPath = normalImage;
...
void MenuScene::createBackground() {
  ...
  if (overlay) {
    overlay->setPosition(Vec2(origin.x + visibleSize.width / 2,
                              origin.y + visibleSize.height / 2));
  ...
  if (overlayHalf) {
    float overlayScale =
        visibleSize.width / overlayHalf->getContentSize().width;
    overlayHalf->setAnchorPoint(Vec2(0.5f, 1.0f));
    overlayHalf->setPosition(
...
void MenuScene::createMenuButtons() {
  ...
  float startX =
      origin.x + (visibleSize.width - totalWidth) / 2 + buttonWidth / 2;
  float y = origin.y + visibleSize.height * 0.18f;

  auto startButton =
      createImageButton(startNormal, startSelected,
                        CC_CALLBACK_1(MenuScene::startGameCallback, this));
  auto loadButton =
      createImageButton(loadNormal, loadSelected,
                        CC_CALLBACK_1(MenuScene::continueGameCallback, this));
  auto coopButton = createImageButton(
      coopNormal, coopSelected, CC_CALLBACK_1(MenuScene::coopCallback, this));
  auto quitButton =
      createImageButton(quitNormal, quitSelected,
...
void MenuScene::createDecorations() {
  ...
  if (clouds) {
    clouds->setPosition(Vec2(origin.x + visibleSize.width * 0.72f,
  ...
  if (cloud) {
    cloud->setPosition(Vec2(origin.x + visibleSize.width * 0.85f,
...
void MenuScene::continueGameCallback(Ref* sender) {
  ...
  if (!SaveManager::getInstance()->hasSaveFile()) {
    ...
    messageBg->setPosition(
        Vec2((visibleSize.width - 450) / 2, (visibleSize.height - 100) / 2));
    this->addChild(messageBg, 100);

    auto label =
        Label::createWithSystemFont("No saved game found!", "Arial", 36);
    label->setPosition(visibleSize.width / 2, visibleSize.height / 2);
    label->setColor(Color3B(255, 255, 100));
    label->enableShadow(Color4B(0, 0, 0, 255), Size(2, -2), 0);
    this->addChild(label, 101);

    auto removeAction = Sequence::create(DelayTime::create(1.5f),
...
void MenuScene::coopCallback(Ref* sender) {
  ...
  messageBg->setPosition(
      Vec2((visibleSize.width - 520) / 2, (visibleSize.height - 100) / 2));
  this->addChild(messageBg, 100);
  ...
  auto removeAction =
      Sequence::create(DelayTime::create(1.0f), RemoveSelf::create(), nullptr);
  messageBg->runAction(removeAction);
  label->runAction(removeAction->clone());
}
