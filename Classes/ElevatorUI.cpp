...
void ElevatorUI::createPanel() {
  ...
  Vec2 vertices[] = {
      Vec2(-halfSize.x, -halfSize.y), Vec2(halfSize.x, -halfSize.y),
      Vec2(halfSize.x, halfSize.y), Vec2(-halfSize.x, halfSize.y)};
  ...
  inputBg->drawSolidRect(Vec2(-100, -25), Vec2(100, 25),
                         Color4F(0, 0, 0, 0.5f));
  ...
  // 提示
  auto hintLabel =
      Label::createWithSystemFont("Type number (0-5) then ENTER", "Arial", 16);
...
void ElevatorUI::createFloorButtons() {
  ...
  std::vector<std::string> floorNames = {u8"[0] farm", u8"[1]", u8"[2]",
                                         u8"[3]",      u8"[4]", u8"[5]"};
...
void ElevatorUI::setFloorSelectCallback(
...
void ElevatorUI::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
  ...
  if (keyCode >= EventKeyboard::KeyCode::KEY_0 &&
    ...
    number = static_cast<int>(keyCode) -
  ...
  if (number != -1) {
    ...
    if (inputText_.length() < 2) {
      // 如果显示的还是错误提示，先清空
      if (inputLabel_->getString() == "Invalid!" ||
          inputLabel_->getString() == "Error") {
        inputText_ = "";
        inputLabel_->setColor(Color3B::WHITE);
      }
