...
Vec2 findNearestWalkableTile(MapLayer* mapLayer, const Vec2& desiredTile) {
  ...
  auto isWalkableTile = [mapLayer](int x, int y) {
    Vec2 pos = mapLayer->tileCoordToPosition(
...
Vec2 findBeachSpawnTile(MapLayer* mapLayer, const Vec2& preferredTile) {
  if (!mapLayer) return preferredTile;

  auto tmxMap = mapLayer->getTMXMap();
  if (!tmxMap) return preferredTile;

  auto waterLayer = tmxMap->getLayer("Water");
  if (!waterLayer) return preferredTile;

  Size mapSize = tmxMap->getMapSize();
  int maxX = static_cast<int>(mapSize.width) - 1;
  int maxY = static_cast<int>(mapSize.height) - 1;
  if (maxX < 0 || maxY < 0) return preferredTile;
  ...
  auto findLandToRightOfWater = [&](int row, Vec2& outTile) -> bool {
    ...
    for (int x = 0; x <= maxX; ++x) {
      if (waterLayer->getTileGIDAt(
              Vec2(static_cast<float>(x), static_cast<float>(row))) != 0) {
        leftmostWater = x;
        break;
      }
    }

    if (leftmostWater < 0) return false;

    int candidateX = leftmostWater;
    while (candidateX <= maxX &&
           waterLayer->getTileGIDAt(Vec2(static_cast<float>(candidateX),
                                         static_cast<float>(row))) != 0) {
      ++candidateX;
    }

    if (candidateX > maxX) return false;
  ...
  for (int offset = 0; offset <= maxY; ++offset) {
    ...
    if (offset > 0 && rowDown >= 0 &&
        findLandToRightOfWater(rowDown, candidate))
      return candidate;
  }
...
BeachScene* BeachScene::createScene(InventoryManager* inventory, int dayCount,
...
bool BeachScene::init(InventoryManager* inventory, int dayCount,
                      float accumulatedSeconds) {
...
void BeachScene::initUI() {
  ...
  timeLabel_->setPosition(
      Vec2(origin.x + 20.0f, origin.y + visibleSize.height - 30.0f));
  ...
  moneyLabel_->setPosition(Vec2(origin.x + visibleSize.width - 20.0f,
  ...
  actionLabel_->setPosition(
      Vec2(origin.x + visibleSize.width / 2, origin.y + 90));
  actionLabel_->setColor(Color3B::WHITE);
  uiLayer_->addChild(actionLabel_, 1);

  auto helpLabel =
      Label::createWithSystemFont("B: Bag | ESC: Back to Farm", "Arial", 14);
...
void BeachScene::initToolbarUI() {
  if (!uiLayer_ || toolbarItems_.empty()) return;
...
void BeachScene::updateUI() {
  if (timeLabel_) {
    ...
    timeLabel_->setString(
        StringUtils::format("Day %d, %02d:%02d", dayCount_, hour, minute));
  }
...
void BeachScene::backToFarm() {
  ...
  Director::getInstance()->replaceScene(
      TransitionFade::create(0.8f, gameScene));
}
...
void BeachScene::showActionMessage(const std::string& text,
                                   const Color3B& color) {
...
void BeachScene::onMouseDown(Event* event) {
  ...
  if (!mouseEvent ||
      mouseEvent->getMouseButton() != EventMouse::MouseButton::BUTTON_LEFT)
    return;
  ...
  if (current != ItemType::FishingRod) return;
...
void BeachScene::startFishing() {
  ...
  std::vector<ItemType> seaFish = {
      ItemType::ITEM_Anchovy,  ItemType::ITEM_Flounder,
      ItemType::ITEM_Tilapia,  ItemType::ITEM_Pufferfish,
      ItemType::ITEM_Sturgeon, ItemType::ITEM_Eel};
