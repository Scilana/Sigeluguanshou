...
MarketUI* MarketUI::create(InventoryManager* inventory,
                           MarketState* marketState, FarmManager* farmManager) {
...
bool MarketUI::init(InventoryManager* inventory, MarketState* marketState,
...
void MarketUI::refresh() {
  if (!inventory_ || !marketState_) return;
  ...
  seasonLabel_->setString(
      StringUtils::format("Season: %s", marketState_->getSeasonName().c_str()));
  weatherLabel_->setString(StringUtils::format(
      "Weather: %s", marketState_->getWeatherName().c_str()));

  const auto& buyGoods = marketState_->getBuyGoods();
  for (size_t i = 0; i < buyLabels_.size() && i < buyGoods.size(); ++i) {
    std::string name = InventoryManager::getItemName(buyGoods[i].itemType);
    buyLabels_[i]->setString(StringUtils::format("%s - %dG", name.c_str(),
                                                 buyGoods[i].currentPrice));
  }
...
void MarketUI::updateHighlight() {
  for (size_t i = 0; i < buyLabels_.size(); ++i) {
    ...
    buyLabels_[i]->setColor(selected ? Color3B(255, 215, 0)
                                     : Color3B(210, 210, 210));
  }
...
void MarketUI::updateInfo() {
  ...
  if (!buyGoods.empty()) {
    ...
    infoLabel_->setString(StringUtils::format("Buy %s for %dG (You have %d)",
                                              name.c_str(), good.currentPrice,
                                              owned));
  } else {
...
void MarketUI::executeTrade() {
  ...
  statusLabel_->setString(
      StringUtils::format("Bought %s (-%dG)", name.c_str(), price));
...
void MarketUI::close() {
  ...
  runAction(Sequence::create(Spawn::create(fadeOut, scaleDown, nullptr),
                             callback, nullptr));
}
