...
QuantityPopup* QuantityPopup::create(int maxVal,
...
void QuantityPopup::createUI() {
  ...
  panel->drawSolidRect(Vec2(-150, -100), Vec2(150, 100),
  ...
  auto subTitle = Label::createWithSystemFont(
      StringUtils::format("(Max: %d)", maxVal_), "Arial", 14);
...
void QuantityPopup::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
  ...
  if (keyCode >= EventKeyboard::KeyCode::KEY_0 &&
    ...
    int num = static_cast<int>(keyCode) -
...
void QuantityPopup::close() { this->removeFromParent(); }
