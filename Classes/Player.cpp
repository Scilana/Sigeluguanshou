...
SpriteFrame* createFullFrame(const std::string& filename) {
  ...
  return SpriteFrame::createWithTexture(texture,
                                        Rect(0, 0, size.width, size.height));
}

void appendFishingFrames(Vector<SpriteFrame*>& frames, const char* dir,
                         const char* stage, int count) {
  for (int i = 1; i <= count; ++i) {
    std::string filename =
        StringUtils::format("tools/fishing/%s%s%d.png", dir, stage, i);
...
void Player::loadAnimations() {
  ...
  auto createAnim = [](std::string actionName, std::string direction,
                       int frameCount) -> Animation* {
    ...
    for (int i = 1; i <= frameCount; i++) {
      ...
      if (actionName == "walk") {
        // 例：角色行走下方向帧序列
        filename = StringUtils::format("characters/player_walk_%s_%d.png",
                                       direction.c_str(), i);
      }
      // 工具动画使用新版命名格式
      else {
        // 例：角色工具下方向帧序列
        filename = StringUtils::format(
            "characters/%s%s%d.png", direction.c_str(), actionName.c_str(), i);
      }
  ...
  switch (state) {
    ...
    case PlayerState::ATTACK: {
      ...
      if (currentToolType_ == ItemType::Hoe) {
        ...
        if (isVertical) {
          targetAnim =
              isUp ? useWateringUpAnimation_ : useWateringDownAnimation_;
        } else {
          targetAnim =
              isRight ? useWateringRightAnimation_ : useWateringLeftAnimation_;
        }
        ...
        if (isVertical) {
          ...
          targetAnim =
              isRight ? usePickaxeRightAnimation_ : usePickaxeLeftAnimation_;
        }
      } else {
        // === 默认/斧头动画 ===
        // （以后如果加入镐子，可在这里增加分支）
    ...
    default:
      // 待机状态：根据朝向显示对应的站立图
      {
        ...
        if (std::abs(dir.y) >= std::abs(dir.x)) {
          standFile = (dir.y > 0) ? "characters/standUp.png"
                                  : "characters/standDown.png";
        } else {
          standFile = (dir.x > 0) ? "characters/standRight.png"
                                  : "characters/standLeft.png";
        }

        // 加载对应的站立图
        if (FileUtils::getInstance()->isFileExist(standFile)) {
          this->setSpriteFrame(SpriteFrame::create(
              standFile, Rect(0, 0, FRAME_WIDTH, FRAME_HEIGHT)));
        }
      }
      return;
  }
  ...
  if (targetAnim) {
    ...
    if (state == PlayerState::ATTACK) {
      ...
      auto callback = CallFunc::create(
          CC_CALLBACK_0(Player::onAttackAnimationFinished, this));
...
void Player::startFishingCast() {
  ...
  auto animation =
      Animation::createWithSpriteFrames(frames, kFishingCastFrameDelay);
...
void Player::startFishingWait() {
  ...
  auto animation =
      Animation::createWithSpriteFrames(frames, kFishingWaitFrameDelay);
  auto animate = Animate::create(animation);
...
void Player::startFishingReel(const std::function<void()>& onFinished) {
  ...
  auto animation =
      Animation::createWithSpriteFrames(frames, kFishingReelFrameDelay);
  auto animate = Animate::create(animation);
...
void Player::update(float delta) {
  ...
  if (isMoving_) {
    ...
    if (std::abs(dir.y) >= std::abs(dir.x)) {
      playAnimation((dir.y > 0) ? PlayerState::WALK_UP
                                : PlayerState::WALK_DOWN);
    } else {
      playAnimation((dir.x > 0) ? PlayerState::WALK_RIGHT
    ...
    auto checkCollision = [this](const Vec2& pos) -> bool {
      ...
      if (farmManager_) {
        ...
        if (farmManager_->getStorageChestAt(tileCoord)) {
          ...
          Vec2 currentTile =
              mapLayer_->positionToTileCoord(this->getPosition());
...
void Player::setMapLayer(MapLayer* mapLayer) { mapLayer_ = mapLayer; }
