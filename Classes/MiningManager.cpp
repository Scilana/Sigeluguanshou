...
MiningManager* MiningManager::create(MineLayer* mineLayer,
...
void MiningManager::initMineralDefs() {
  ...
  for (int gid = 870; gid <= 880; ++gid) {
    mineralDefs_[gid] =
        MineralDef{gid, "Copper Ore", 3, ItemType::CopperOre, 1, 3, 5};
  }
  ...
  for (int gid = 881; gid <= 890; ++gid) {
    mineralDefs_[gid] =
        MineralDef{gid, "Iron Ore", 4, ItemType::IronOre, 1, 2, 8};
  }
  ...
  for (int gid = 900; gid <= 920; ++gid) {
    mineralDefs_[gid] =
        MineralDef{gid, "Silver Ore", 5, ItemType::SilverOre, 1, 2, 10};
  }
  ...
  for (int gid = 649; gid <= 723; ++gid) {
    mineralDefs_[gid] =
        MineralDef{gid, "Gold Ore", 8, ItemType::GoldOre, 1, 1, 20};
  }

  // 钻石矿：编号 800-810
  for (int gid = 800; gid <= 810; ++gid) {
    mineralDefs_[gid] =
        MineralDef{gid, "Diamond Ore", 10, ItemType::DiamondOre, 1, 1, 50};
  }
}
...
MiningManager::MiningResult MiningManager::mineTile(const Vec2& tileCoord) {
  ...
  if (it == activeMinings_.end()) {
    ...
    if (progress.requiredHits <= 1) {
      ...
      return {true, "Mining... (" + std::to_string(progress.requiredHits - 1) +
                        " more)"};
    }
  } else {
    // 继续敲击
...
void MiningManager::addExp(int exp) { miningExp_ += exp; }
...
std::string MiningManager::dropItems(const Vec2& tileCoord,
                                     const MineralDef& mineralDef) {
  if (mineralDef.dropItem == ItemType::ITEM_NONE) return "";

  // 计算掉落数量
  int dropCount = mineralDef.dropMinCount;
  if (mineralDef.dropMaxCount > mineralDef.dropMinCount) {
    dropCount +=
        rand() % (mineralDef.dropMaxCount - mineralDef.dropMinCount + 1);
  }
