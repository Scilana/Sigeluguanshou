...
bool SkillTreeUI::init() {
  ...
  auto title =
      Label::createWithSystemFont("Skill Tree (E to close)", "Arial", 24);
  title->setPosition(Vec2(panel_->getContentSize().width / 2,
                          panel_->getContentSize().height - 30));
...
void SkillTreeUI::buildLayout() {
  ...
  for (size_t i = 0; i < list.size(); ++i) {
    ...
    auto barBg =
        LayerColor::create(Color4B(80, 80, 80, 255), barWidth, barHeight);
...
void SkillTreeUI::close() { removeFromParent(); }

void SkillTreeUI::updateRow(SkillRow& row,
                            const SkillManager::SkillData& data) {
  if (row.nameLabel) row.nameLabel->setString(data.name);

  if (row.levelLabel) {
    std::string levelText =
        StringUtils::format("Lv %d/%d", data.level, data.maxLevel);
    row.levelLabel->setString(levelText);
  }

  if (row.countLabel) {
    std::string countText =
        StringUtils::format("Actions: %d", data.actionCount);
    row.countLabel->setString(countText);
  }

  float progress =
      SkillManager::getInstance()->getProgressToNextLevel(row.type);
  float fillWidth = row.barWidth * progress;
  if (row.barFill) {
    row.barFill->setContentSize(
        Size(fillWidth, row.barFill->getContentSize().height));
  }
}
