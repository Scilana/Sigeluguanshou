...
void TreasureChest::initDisplay() {
  ...
  // 绘制箱体 (底座) - 深棕色
  Vec2 bodyVertices[] = {Vec2(-14, -10), Vec2(14, -10), Vec2(14, 6),
                         Vec2(-14, 6)};
  Color4F bodyColor(0.55f, 0.27f, 0.07f, 1.0f);
  displayNode_->drawPolygon(bodyVertices, 4, bodyColor, 1,
                            Color4F(0.3f, 0.15f, 0.05f, 1.0f));

  // 绘制盖子 -稍亮的棕色
  Vec2 lidVertices[] = {Vec2(-15, 6), Vec2(15, 6), Vec2(15, 14), Vec2(-15, 14)};
  Color4F lidColor(0.65f, 0.35f, 0.1f, 1.0f);
  displayNode_->drawPolygon(lidVertices, 4, lidColor, 1,
                            Color4F(0.4f, 0.2f, 0.05f, 1.0f));

  // 金色镶边 (竖条)
  displayNode_->drawSolidRect(Vec2(-10, -10), Vec2(-7, 14),
                              Color4F(1.0f, 0.84f, 0.0f, 1.0f));  // 左
  displayNode_->drawSolidRect(Vec2(7, -10), Vec2(10, 14),
                              Color4F(1.0f, 0.84f, 0.0f, 1.0f));  // 右

  // 锁扣 (中间)
  displayNode_->drawSolidRect(Vec2(-3, 2), Vec2(3, 8),
                              Color4F(0.8f, 0.8f, 0.8f, 1.0f));  // 银色锁底
...
TreasureChest::LootResult TreasureChest::generateLoot() {
  ...
  if (roll < weaponChance) {
    ...
    if (floorLevel_ >= 3 && resourceRoll < 30) {
      ...
      result.message =
          StringUtils::format("Found %d Silver Ore!", result.count);
    } else {
      // 铜矿
      result.item = ItemType::CopperOre;
      result.count = 2 + rand() % 4;  // 2-5个
      result.message =
          StringUtils::format("Found %d Copper Ore!", result.count);
    }
  }
...
void TreasureChest::playOpenAnimation() {
  ...
  if (displayNode_) {
    ...
    // 绘制内部 (黑色/深色) - 表示空了
    displayNode_->drawSolidRect(Vec2(-12, -4), Vec2(12, 6),
                                Color4F(0.1f, 0.05f, 0.0f, 1.0f));

    // 绘制盖子 (向后翻/移开)
    // 这里简单地画一个变暗的盖子在上方，表示打开
    Vec2 lidVertices[] = {Vec2(-15, 14), Vec2(15, 14), Vec2(15, 20),
                          Vec2(-15, 20)};
