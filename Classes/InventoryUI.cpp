...
void InventoryUI::setSelectionMode(bool enabled) { _selectionMode = enabled; }

void InventoryUI::setOnItemSelectedCallback(
    const ItemSelectCallback& callback) {
...
void InventoryUI::initBackground() {
  ...
  // 半透明黑色背景
  background_ = LayerColor::create(Color4B(0, 0, 0, 180), visibleSize.width,
                                   visibleSize.height);
...
void InventoryUI::initPanel() {
  ...
  float panelHeight =
      ROWS * (SLOT_SIZE + SLOT_SPACING) + SLOT_SPACING * 2 + 120;

  // 面板背景（深色）
  panel_ =
      LayerColor::create(Color4B(40, 35, 30, 255), panelWidth, panelHeight);
  panel_->setPosition(Vec2((visibleSize.width - panelWidth) / 2,
                           (visibleSize.height - panelHeight) / 2));
  this->addChild(panel_, 1);

  // 面板边框
  auto border = DrawNode::create();
  border->drawRect(Vec2(0, 0), Vec2(panelWidth, panelHeight),
...
void InventoryUI::initSlots() {
  ...
  for (int row = 0; row < ROWS; ++row) {
    for (int col = 0; col < COLS; ++col) {
      ...
      if (slotIndex >= inventory_->getSlotCount()) break;
...
void InventoryUI::initControls() {
  ...
  // 关闭按钮提示
  auto hint =
      Label::createWithSystemFont("Press ESC or B to close", "Arial", 18);
...
cocos2d::Sprite* InventoryUI::createSlotBackground() {
  ...
  // 边框
  auto border = DrawNode::create();
  border->drawRect(Vec2(0, 0), Vec2(SLOT_SIZE, SLOT_SIZE),
...
cocos2d::Sprite* InventoryUI::createItemIcon(
    const InventoryManager::ItemSlot& slot) {
  ...
  if (slot.isTool() && slot.maxDurability > 0) {
    const float percent =
        static_cast<float>(slot.durability) / slot.maxDurability;
    ...
    if (percent < 0.2f)
      ...
      color = Color4F(1.0f, 0.8f, 0.0f, 1.0f);
...
void InventoryUI::onSlotClicked(int slotIndex) {
  ...
  if (selectedSlotIndex_ != -1 &&
      inventory_->getSlot(selectedSlotIndex_).isEmpty()) {
  ...
  if (selectedSlotIndex_ == -1) {
    if (clickedSlot.isEmpty()) {
      if (clearedInvalidSelection) {
        this->scheduleOnce([this](float) { updateSelection(); }, 0,
                           "deferred_update_selection");
      }
      return;
    }
    ...
    std::string info = StringUtils::format(
        "%s x%d - %s", InventoryManager::getItemName(clickedSlot.type).c_str(),
        clickedSlot.count,
  ...
  if (!newSlot.isEmpty()) {
    std::string info = StringUtils::format(
        "%s x%d - %s", InventoryManager::getItemName(newSlot.type).c_str(),
        newSlot.count,
...
void InventoryUI::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
  ...
  if (keyCode >= EventKeyboard::KeyCode::KEY_0 &&
    ...
    const int num = static_cast<int>(keyCode) -
                    static_cast<int>(EventKeyboard::KeyCode::KEY_0);
    int targetSlotIndex =
        (num == 0) ? 9 : num - 1;  // 数字键映射：1→0，2→1，…，0→9
    ...
    if (selectedSlotIndex_ != -1) {
      if (selectedSlotIndex_ != targetSlotIndex) {
        ...
        // 显示提示
        if (infoLabel_) {
          std::string itemName = InventoryManager::getItemName(
              inventory_->getSlot(targetSlotIndex).type);
          infoLabel_->setString(StringUtils::format("Moved %s to slot %d",
                                                    itemName.c_str(), num));
        }
...
void InventoryUI::handleTransfer() {
  ...
  if (partnerIsShippingBin_) {
    ...
    auto doTransfer = [this, price](int count) {
      if (this->partnerInventory_->addItem(
              this->inventory_->getSlot(this->selectedSlotIndex_).type,
              count)) {
        std::string itemName = InventoryManager::getItemName(
            this->inventory_->getSlot(this->selectedSlotIndex_).type);
        ...
        this->infoLabel_->setString(StringUtils::format(
            "Placed %s in bin (+%dG)", itemName.c_str(), totalValue));
      } else {
    ...
    if (slot.count > 1) {
      ...
      auto popup = QuantityPopup::create(slot.count, [this, currentIndex, price,
                                                      doTransfer](int count) {
        ...
        if (this->partnerInventory_->addItem(verifySlot.type, count)) {
          ...
          this->refresh();
          this->infoLabel_->setString(StringUtils::format(
              "Placed %s in bin (+%dG)", itemName.c_str(), price * count));
        } else {
          this->infoLabel_->setString("Shipping bin full!");
        }
      });
  ...
  if (slot.count > 1) {
    int currentIndex = selectedSlotIndex_;
    auto popup =
        QuantityPopup::create(slot.count, [this, currentIndex](int count) {
